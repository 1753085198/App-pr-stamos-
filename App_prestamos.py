...
def generar_excel_de_gala(datos_c, historial_c):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        ws1["A1"] = "REPORTE DE CUENTA"; ws1["A1"].font = Font(bold=True, size=16)
        ws1["A3"] = "CLIENTE:"; ws1["B3"] = str(datos_c['Nombre']).upper()
        ws1["D3"] = "SALDO:"; ws1["E3"] = f"${datos_c['Saldo_Restante']}"
        h_plan = ["N° Cuota", "Descripción", "Valor Cuota", "Estado"]
        ...
        for idx, r_data in activos.iterrows():
            ...
            with st.expander(f"👤 {r_data['Nombre'].upper()}  |  💰 SALDO: ${r_data['Saldo_Restante']}", expanded=is_open):
                ...
                with col1:
                    ...
                    st.download_button(f"📥 EXCEL ESTADO", data=generar_excel_de_gala(r_data, h_c), file_name=f"Reporte_{r_data['Nombre']}.xlsx", key=f"ex_{r_data['ID']}", use_container_width=True)
