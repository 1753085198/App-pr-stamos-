...
# 2. CSS PARA INTERFAZ ULTRA-GIGANTE AJUSTADA
st.markdown("""
    <style>
    ...
    /* BUSCADOR (Letra un poco más pequeña como pediste) */
...
# --- GENERADOR DE EXCEL ACTUALIZADO ---
def generar_excel_de_gala(datos_c, historial_c):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        font_white = Font(color="FFFFFF", bold=True); font_green = Font(color="006100", bold=True)
        ...
        headers_plan = ["N° Cuota", "Descripción", "Valor Cuota ($)", "Estado Actual"]
...
with t_gestion:
    busq = st.text_input("🔍 BUSCAR POR NOMBRE O CÉDULA:", placeholder="Escribe aquí...")
        ...
        for idx, r_data in activos.iterrows():
            ...
            with st.expander(f"👤 {r_data['Nombre'].upper()}  |  💰 SALDO: ${r_data['Saldo_Restante']}", expanded=is_open):
                ...
                with col1:
                    ...
                    st.metric("AVANCE", f"{r_data['Pagos_Realizados']}/{r_data['Meses_Totales']}")
                    h_c = df_h[df_h["ID_Prestamo"] == r_data['ID']] if df_h is not None else pd.DataFrame()
                    st.download_button(f"📥 DESCARGAR ESTADO", data=generar_excel_de_gala(r_data, h_c), file_name=f"Estado_{r_data['Nombre']}.xlsx", key=f"ex_{r_data['ID']}_{st.session_state.pago_key}", use_container_width=True)

                with col2:
                    st.write("### 💵 REGISTRAR ABONO")
                    with st.form(key=f"f_{r_data['ID']}_{st.session_state.pago_key}"):
                ...
                st.write("---")
                if st.button("🗑️ ELIMINAR NOMBRE PERMANENTEMENTE", key=f"trash_{r_data['ID']}", use_container_width=True, type="secondary"):
