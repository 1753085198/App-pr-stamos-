...
st.markdown("""
    <style>
    button[data-baseweb="tab"] { font-size: 24px !important; font-weight: 600 !important; padding: 1.2rem !important; }
    .stMarkdown p, label, .stSelectbox p, .stNumberInput label { font-size: 21px !important; font-weight: 500 !important; }
    .stButton>button { font-size: 24px !important; font-weight: bold !important; height: 4rem !important; border-radius: 12px !important; }
    [data-testid="stMetricValue"] { font-size: 48px !important; }
    .stDataFrame { font-size: 19px !important; }
    /* Estilo para el botón de eliminar */
    .stButton>button[kind="secondary"] { color: white !important; background-color: #ff4b4b !important; border: none !important; }
    </style>
...
with tab3:
    if not df_p.empty:
        ...
        st.download_button(f"📥 Descargar Excel de {c_datos['Nombre']}", data=generar_excel_cliente(c_datos, c_historial), file_name=f"Control_{id_sel}.xlsx", use_container_width=True)

        st.divider()
        st.write("### ⚠️ Zona de Peligro")
        if st.button(f"🗑️ ELIMINAR PRESTAMO DE {c_datos['Nombre']}", use_container_width=True, type="secondary"):
            with st.spinner('Borrando de todas las hojas...'):
                # Borrar de Prestamos
                ...
                st.warning("Registro y bitácora eliminados.")
...
with tab4:
    ...
    if not activos.empty:
        ...
        with c_p2: foto = st.file_uploader("📸 Recibo (JPG/PNG):", type=["jpg", "png", "jpeg"])
